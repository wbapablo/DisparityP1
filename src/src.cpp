...
int main(){
    ...
    Mat roi8 = disp8(Rect(26,0,672-26,376));
    cv::resize(roi8,roi8,img_size);
    imshow("Disparity Normalized" , roi8);

    waitKey(0);
	/*
    cout << "Disp8 type " << disp8.type() << endl;
    cout << "Disp type  " << disp.type() << endl;
    cout << "Disp8 size " << disp8.size() << endl;
    cout << "Disp size  " << disp.size() << endl;

    Mat roi = disp(Rect(26,0,672-26,376));
    cv::resize(roi,roi,img_size);
    imshow("Raw Disparity" , roi);
    ...
    for(int y=0; y<roi.rows;y++){
    	for(int x=0;x<roi.cols;x++){
    		...
    		//Normalize with scaling value
    		vec_temp /= vec_temp(3);

    		if(isnan(roi.at<uchar>(y,x)))
    			cout<<"Hey" << endl << vec_temp << endl;
    		//Add World Coordinate to PCL cloud 

    		o.x = vec_temp(0);
    		o.y = vec_temp(1);
    		o.z = vec_temp(2);
    		
    		o.r = imgLr.at<Vec3b>(y,x)[2];
    		o.g = imgLr.at<Vec3b>(y,x)[1]; 
    		o.b = imgLr.at<Vec3b>(y,x)[0]; 
    		//Add World coordinate to PCL cloud
    ...
    vis = rgbVis(cloud);
    pcl::io::savePCDFile("cartulinas.pcd" , *cloud,true);
    std::cerr << "Saved " << cloud->size () << " data points to cartulinas.pcd." << std::endl;
    char key = ' ';
    while (key != 'q')
    {
    	vis->spinOnce(5);
...
boost::shared_ptr<pcl::visualization::PCLVisualizer> rgbVis(pcl::PointCloud<pcl::PointXYZRGB>::ConstPtr cloud) {
    ...
    boost::shared_ptr<pcl::visualization::PCLVisualizer> viewer(new pcl::visualization::PCLVisualizer("PCL ZED 3D Viewer"));
    ...
    viewer->setCameraPosition(9.27381, -11.099, 2.2511,0.148702, -6.12353, -0.471121, -0.312329, -0.053896, 0.948444,0);
